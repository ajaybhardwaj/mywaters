...
- (void)loadImageWithURL:(NSURL *)URL target:(id)target action:(SEL)action failure:(SEL)failure
{
    [self loadImageWithURL:URL target:target success:action failure:failure];
}
...
{
    //Modify by Jack(for place holder image)
	[[AsyncImageLoader sharedLoader] loadImageWithURL:imageURL target:self action:@selector(setImage:) failure:@selector(setImageFailed)];
}
...
- (void)setImage:(UIImage *)image
{
  ...
  /*
   remove Raja's implementation of Placeholder image from here
   */
//  if (!image) {
//    return;
//  }
//  if ([self viewWithTag:PLACEHOLDER_IMG_CONSTANT]) {
//    [[self viewWithTag:PLACEHOLDER_IMG_CONSTANT] removeFromSuperview];
//  }
    
    // -- don't remove now to see the lustre lounge image place holder
...
- (void)dealloc
{
    ...
    if (_retainedImageURL) {
        [_retainedImageURL release]; _retainedImageURL = nil;
    }
